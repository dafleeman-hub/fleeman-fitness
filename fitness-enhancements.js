...
function injectFitnessEnhancementUi() {
  ...
  if (!document.querySelector("#restTimerPanel")) document.body.insertAdjacentHTML("beforeend", `
    <section id="restTimerPanel" class="rest-timer-overlay hidden" role="dialog" aria-label="Rest timer" aria-live="polite" aria-hidden="true">
      <div class="rest-timer-overlay-card">
        <button id="restTimerCollapse" class="rest-timer-collapse" type="button" aria-label="Collapse rest timer">−</button>
        <span class="eyebrow">RESTING</span><strong id="restTimerValue">00:00</strong>
        <p id="restTimerContext" class="rest-timer-context"></p>
        <div class="rest-timer-next"><span>NEXT</span><strong id="restTimerNextSet">Next set</strong><small id="restTimerNextDetails"></small></div>
        <div class="rest-timer-actions"><button id="restTimerAdd15" class="secondary-button compact" type="button">+15 sec</button><button id="restTimerPause" class="secondary-button compact" type="button">Pause</button><button id="restTimerAdd30" class="secondary-button compact" type="button">+30 sec</button><button id="restTimerSkip" class="secondary-button compact" type="button">Skip Rest</button><button id="restTimerRestart" class="secondary-button compact" type="button">Restart</button></div>
      </div>
    </section>
    <button id="restTimerMini" class="rest-timer-mini hidden" type="button" aria-label="Open rest timer"><span>REST</span><strong id="restTimerMiniValue">00:00</strong><small id="restTimerMiniExercise"></small></button>`);
...
function restTimerInteraction(timer=currentSession?.restTimer,duration=3200){if(!timer)return;timer.uiMode="expanded";timer.interactionUntil=Date.now()+duration;timer.collapseAt=Date.now()+Math.max(2500,duration);}
function expandRestTimer(timer=currentSession?.restTimer,manual=true){if(!timer)return;timer.uiMode="expanded";timer.collapseAt=Date.now()+(manual?5000:2500);timer.interactionUntil=manual?Date.now()+3000:0;persistActiveWorkout(true);updateRestTimerUi();}
function collapseRestTimer(timer=currentSession?.restTimer){if(!timer)return;timer.uiMode="compact";timer.interactionUntil=0;persistActiveWorkout(true);updateRestTimerUi();}
function completeRestTimer(timer){timer.status="complete";timer.remainingSeconds=0;timer.uiMode="expanded";timer.collapseAt=Date.now()+2500;if(timer.completionAlerted)return;timer.completionAlerted=true;if(data.settings.restTimerAlerts.vibration&&navigator.vibrate)navigator.vibrate([180,80,180]);playRestCompletionTone();persistActiveWorkout(true);}
function updateRestTimerUi(){
  const panel=document.querySelector("#restTimerPanel"),mini=document.querySelector("#restTimerMini");if(!panel||!mini)return;
  const timer=currentSession?.restTimer;if(!timer){panel.classList.add("hidden");panel.setAttribute("aria-hidden","true");mini.classList.add("hidden");return;}
  timer.uiMode||="compact";timer.interactionUntil=Number(timer.interactionUntil)||0;timer.collapseAt=Number(timer.collapseAt)||0;
  const remaining=restTimerRemaining(timer);if(timer.status==="running"&&remaining<=0)completeRestTimer(timer);
  if(timer.uiMode==="expanded"&&timer.status!=="paused"&&Date.now()>=timer.collapseAt&&Date.now()>=timer.interactionUntil)timer.uiMode="compact";
  const expanded=timer.uiMode==="expanded";panel.classList.toggle("hidden",!expanded);panel.setAttribute("aria-hidden",String(!expanded));mini.classList.toggle("hidden",expanded);
  panel.classList.toggle("complete",timer.status==="complete");mini.classList.toggle("complete",timer.status==="complete");
  const value=timer.status==="complete"?"READY":formatRestTimer(restTimerRemaining(timer));
  document.querySelector("#restTimerValue").textContent=value;document.querySelector("#restTimerMiniValue").textContent=value;document.querySelector("#restTimerMiniExercise").textContent=timer.exerciseName;
  document.querySelector("#restTimerContext").textContent=timer.status==="complete"?`Rest complete • ${timer.exerciseName}`:`${timer.exerciseName} • Set ${timer.afterSet} complete`;
  document.querySelector("#restTimerNextSet").textContent=timer.nextSetLabel||"Next exercise";document.querySelector("#restTimerNextDetails").textContent=timer.nextDetails||"Continue when ready";
  document.querySelector("#restTimerPause").textContent=timer.status==="paused"?"Resume":"Pause";
}
...
function startExerciseRestTimer(exercise,setIndex){const {prescription,definition}=sessionExerciseDefinition(exercise),duration=Math.max(0,Number(prescription.rest??definition.defaults?.restSeconds??data.settings.rest));if(!duration)return;const nextSet=exercise.sets.findIndex((set,index)=>index>setIndex&&!set.done),minimum=prescription.minReps||definition.defaults?.minReps,maximum=prescription.maxReps||definition.defaults?.maxReps;const nextSetLabel=nextSet>=0?`Set ${nextSet+1}`:"Next exercise",nextLoad=nextSet>=0?workoutLoadText(exercise,exercise.sets[nextSet].weight??exercise.weight):"",nextDetails=nextSet>=0?`${nextLoad} • ${minimum}-${maximum} ${exerciseRepLabel(exercise)}`:"Continue when ready";currentSession.restTimer={status:"running",exerciseId:exercise.exerciseId,exerciseName:exercise.name,afterSet:setIndex+1,durationSeconds:duration,remainingSeconds:duration,startedAt:new Date().toISOString(),endsAt:new Date(Date.now()+duration*1000).toISOString(),nextSetLabel,nextDetails,uiMode:"expanded",collapseAt:Date.now()+2500,interactionUntil:0,completionAlerted:false};persistActiveWorkout(true);updateRestTimerUi();ensureRestTimerTicker();}
...
function syncRestTimerViewport(){const viewport=window.visualViewport,panel=document.querySelector("#restTimerPanel"),mini=document.querySelector("#restTimerMini");if(!panel||!mini)return;const top=viewport?.offsetTop||0,height=viewport?.height||window.innerHeight,keyboardOffset=Math.max(0,window.innerHeight-height-top);panel.style.setProperty("--rest-viewport-top",`${top}px`);panel.style.setProperty("--rest-viewport-height",`${height}px`);mini.style.setProperty("--rest-keyboard-offset",`${keyboardOffset}px`);}
function bindRestTimerControls(){
  const panel=document.querySelector("#restTimerPanel");
  panel.addEventListener("pointerdown",()=>restTimerInteraction());panel.addEventListener("focusin",()=>restTimerInteraction());panel.addEventListener("mouseenter",()=>restTimerInteraction(currentSession?.restTimer,4000));
  document.querySelector("#restTimerMini").onclick=()=>expandRestTimer();document.querySelector("#restTimerCollapse").onclick=()=>collapseRestTimer();
  document.querySelector("#restTimerPause").onclick=()=>{const timer=currentSession?.restTimer;if(!timer||timer.status==="complete")return;if(timer.status==="paused"){timer.status="running";timer.endsAt=new Date(Date.now()+Number(timer.remainingSeconds)*1000).toISOString();timer.collapseAt=Date.now()+2500;timer.interactionUntil=Date.now()+1200;}else{timer.remainingSeconds=restTimerRemaining(timer);timer.status="paused";timer.endsAt=null;timer.uiMode="expanded";timer.interactionUntil=Number.MAX_SAFE_INTEGER;}persistActiveWorkout(true);updateRestTimerUi();};
  document.querySelector("#restTimerSkip").onclick=()=>{if(!currentSession?.restTimer)return;currentSession.restTimer=null;persistActiveWorkout(true);updateRestTimerUi();};
  document.querySelector("#restTimerAdd15").onclick=()=>{restTimerInteraction();adjustRestTimer(15);};document.querySelector("#restTimerAdd30").onclick=()=>{restTimerInteraction();adjustRestTimer(30);};
  document.querySelector("#restTimerRestart").onclick=()=>{const timer=currentSession?.restTimer;if(!timer)return;timer.status="running";timer.remainingSeconds=timer.durationSeconds;timer.startedAt=new Date().toISOString();timer.endsAt=new Date(Date.now()+timer.durationSeconds*1000).toISOString();timer.completionAlerted=false;restTimerInteraction(timer);persistActiveWorkout(true);updateRestTimerUi();};
  syncRestTimerViewport();window.visualViewport?.addEventListener("resize",syncRestTimerViewport);window.visualViewport?.addEventListener("scroll",syncRestTimerViewport);window.addEventListener("resize",syncRestTimerViewport);
}
