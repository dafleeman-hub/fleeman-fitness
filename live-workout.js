...
function collapsedExerciseSummary(exercise) {
  ...
  const weights = [...new Set(done.map(set => displayWeightValue(set.weight,data.profile?.units)))];
  ...
  return `<span class="exercise-summary-copy"><strong>${escapeHtml(exercise.name)}${completed ? " ✓" : ""}</strong><span>${exercise.skipped ? `Skipped${exercise.skipReason ? ` - ${escapeHtml(exercise.skipReason)}` : ""}` : completed ? `${done.length} sets - ${weights.join("-") || displayWeightValue(exercise.weight,data.profile?.units)} ${weightUnit(data.profile?.units)} - ${totalReps} total reps` : `${done.length} of ${exercise.sets.length} sets - ${displayWeightValue(exercise.weight,data.profile?.units)} ${weightUnit(data.profile?.units)}`}</span>${warning?'<span class="exercise-warning-label">Joint-pain adjustment</span>':""}${completed?`<span>Difficulty: ${exercise.feedbackAnswered?feedbackLabel(exercise.feedback):"Pending"} - Joint pain: ${exercise.jointPainAnswered?(exercise.jointPain?.rating??1):"Pending"}</span>`:""}</span><span class="exercise-summary-chevron" aria-hidden="true">${exercise.expanded?"−":"+"}</span>`;
}
...
function setCardMarkup(exercise, set, setIndex, previous, unit, entryLabel) {
  ...
  const priorText = previousSet ? `${displayWeightValue(previousSet.weight ?? previous.result.weight,data.profile?.units)} ${unit} × ${previousSet.reps}` : "No matching previous set";
  return `<article class="live-set-card ${set.done?"completed":""}" data-set-index="${setIndex}"><div class="live-set-card-heading"><strong>Set ${setIndex+1}${set.done?" ✓":""}</strong><span>Previous: ${priorText}</span></div><div class="set-input-grid"><label>${escapeHtml(entryLabel)} (${unit})<input class="set-weight-input" type="number" inputmode="decimal" min="0" step="${data.profile?.units==="metric"?.5:2.5}" value="${displayWeightValue(set.weight,data.profile?.units)}"></label><label>Repetitions<input class="set-reps-input" type="number" inputmode="numeric" min="0" max="100" value="${set.reps}"></label></div><button class="${set.done?"secondary-button":"primary-button"} complete-set-card" type="button" aria-label="${set.done?"Undo":"Complete"} set ${setIndex+1}">${set.done?"Undo completed set":"Complete Set"}</button></article>`;
}
