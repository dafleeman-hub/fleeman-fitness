...
function newMesocycle() {
  const today = FleemanSchedule.dateKey(new Date());
  return {
...
function renderMesoBasics(body) {
  ...
  document.querySelectorAll(".meso-date-choice").forEach(button=>button.onclick=()=>{document.querySelector("#mesoStartDate").value=button.dataset.dateChoice==="monday"?FleemanSchedule.nextMondayKey(new Date()):FleemanSchedule.dateKey(new Date());});
}
...
function moveMesoForward(meso){meso.schedule.forEach(slot=>slot.dayIndex=(slot.dayIndex+1)%7);const date=FleemanSchedule.localDate(meso.startDate);date.setDate(date.getDate()+1);meso.startDate=FleemanSchedule.dateKey(date);saveData();}
