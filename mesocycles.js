...
function copyExercise(exercise) {
  return {
    id: crypto.randomUUID(), name: exercise.name || "", muscle: exercise.muscle || inferExerciseMuscle(exercise.name),
...
function renderMesoWorkouts(body) {
  ...
  mesoBuilder.schedule.forEach((slot, dayIndex) => {
    ...
    card.innerHTML = `<p class="eyebrow">${mesoWeekdays[slot.dayIndex]}</p>
      <label>Use saved workout<select class="template-select"><option value="">Keep current</option>${data.workouts.map(w=>`<option value="${escapeHtml(w.id)}">${escapeHtml(w.name)}</option>`).join("")}</select></label>
      <label>Workout name<input class="day-name" value="${escapeHtml(slot.workout.name)}"></label>
      <label>Notes<input class="day-notes" value="${escapeHtml(slot.workout.notes||"")}"></label>
      <div class="meso-exercises"></div><button class="secondary-button add-meso-exercise">Add exercise</button>`;
    card.querySelector(".template-select").onchange = e => { const w=data.workouts.find(x=>x.id===e.target.value); if(w){slot.workout={id:crypto.randomUUID(),name:w.name,notes:w.notes||"",exercises:w.exercises.map(copyExercise)};renderMesoBuilder();} };
    card.querySelector(".day-name").oninput = e => slot.workout.name=e.target.value;
...
function mesoExerciseEditor(exercise, slot, index) {
  ...
  card.innerHTML=`<label>Exercise<input data-field="name" value="${escapeHtml(exercise.name)}" placeholder="Exercise name"></label>
...
function workoutForMesoSlot(meso,next){let workout=structuredClone(next.plan.workout);if(isDeloadWeek(meso,next.week)){workout.id=`${workout.id}-deload-${meso.id}`;workout.name=`Deload — ${workout.name}`;workout.exercises=workout.exercises.map(e=>({...e,id:`${e.id}-deload`,sets:Math.ceil(e.sets/2),startWeight:Math.round(e.startWeight*.85/2.5)*2.5,targetRir:Math.max(4,e.targetRir)}));}ensureWorkoutTemplate(workout);return workout;}
...
function renderMesocycleToday(){ensureMesocycleData();const meso=data.mesocycles.active,btn=document.querySelector("#startWorkoutButton");if(!meso){btn.onclick=()=>{if(data.selectedWorkoutId)startWorkout(data.selectedWorkoutId);else document.querySelector('[data-view="builderView"]').click();};return;}const next=nextMesoSlot(meso),hero=document.querySelector("#homeView .hero-card");if(!next){hero.querySelector(".eyebrow").textContent="MESOCYCLE COMPLETE";document.querySelector("#todayWorkoutName").textContent=meso.name;document.querySelector("#todayWorkoutSummary").textContent="Open Programs to review and complete this mesocycle.";btn.textContent="Open Programs";btn.onclick=()=>document.querySelector('[data-view="programsView"]').click();return;}const today=new Date().getDay(),scheduledToday=next.plan.dayIndex===today;hero.querySelector(".eyebrow").textContent=`${meso.name.toUpperCase()} • WEEK ${next.week}`;document.querySelector("#todayWorkoutName").textContent=scheduledToday?next.plan.workout.name:"Rest Day";document.querySelector("#todayWorkoutSummary").textContent=scheduledToday?"Today's scheduled mesocycle workout.":`Next scheduled workout: ${next.plan.workout.name}. You can complete it early or choose another workout below.`;btn.textContent=scheduledToday?"Start workout":"Complete next workout early";btn.onclick=()=>startNextMesoWorkout(meso);}
